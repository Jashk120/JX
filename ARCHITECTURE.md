...
- `protocol/primitives` → vocab, `protocol/crypto` → signing/membership, `protocol/consensus` → Hashgraph ordering, `protocol/storage|stream` + `executor/state` → persistence, `protocol/gossip` → pinned-TLS TCP gossip with bounded concurrent fanout, `node` → daemon lifecycle.
- One gossip interval fans out to `k = FanoutMode::effective_k(N)` peers concurrently via `tokio::JoinSet` + `Semaphore(k)` (`ratio 0.6@N≤10 → 0.3@N≥30`, `k_max 4@N≤6, 17@7≤N≤99 Hedera cap, 12@N≥100` — `N=6→4, 10→6, 29→9` computed vs cap `17`, `100→12`), `PeerManager::pick_k` scored selection, `LruCache` hot-pool (`10@N=6, 30@N=100`, LRU eviction), per-peer `DedupState` (`self 1000 ms / ancestor 250 ms / non-ancestor 3000 ms`) in `delta_events_filtered`, `GossipMetrics` (`p50/p95_rtt`, `cache_hit_rate`), pinned-TLS TCP hot-pool (`TcpTransport` + `rustls` SPKI pin). Each sync: `SyncRequest(known frontier)` → `SyncResponse(topo-sorted, dedup-filtered delta)` → verify+insert → create own `Event(self_parent, other_parent)` → push back.
