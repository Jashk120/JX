...
impl fmt::Display for ConsensusError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::AlreadyPresent(hash) => write!(f, "event {hash:?} is already present in the hashgraph"),
            Self::MissingParent(hash) => write!(f, "parent {hash:?} is not present in the hashgraph"),
