...
│    STREAM_VERSION 4: .esf (+.esf_sig) + .rsf (+.rsf_proofs) +   │
│    Merkle records_root + 200B chained checkpoint + state_diffs  │
...
- **D** — produce a chained signed checkpoint per newly decided round
   (`produce_checkpoint` → 200-byte `round||records_root||state_hash||roster_hash||prev_checkpoint_hash||window_root||roster_history_root` with Merkle `records_root` and PLAN-2 Rule 1 `prev` chaining); when >2/3 of members' signatures accumulate in a
